  ...
  R6::R6Class(
    ...
    public = list(
      ...
      initialize = function(muestra = NA,
        ...
        self$n_simulaciones <- if("logical" %in% class(bd_respuestas)) n_simulaciones else 0
       ...
       # self$auditoria_telefonica <- auditoria_telefonica %>% distinct(SbjNum, .keep_all = T)
        ...
        if(!("data.frame" %in% class(bd_respuestas)) & !is.null(n_simulaciones)){
          bd_respuestas <- self$simular_surveytogo(cuestionario = self$cuestionario,
        ...
        self$respuestas <- Respuestas_svy$new(bd_respuestas = bd_respuestas)
        ...
        print(glue::glue("La base de campo contiene ", as.character(nrow(bd_respuestas)), " filas"))
